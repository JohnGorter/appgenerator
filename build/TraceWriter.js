export class TraceWriter {
    ...
    constructor(mode, category, area) {
        ...
        if (!mode)
            this.mode = TraceWriter.DISABLED;
        else
            this.mode = mode;
                ...
                if (area & this.area || category == TraceWriter.ERROR) {
                    switch (category) {
                        case TraceWriter.ERROR: {
                            console.log(`\x1b[31m\x1b[1m ◉  TRACE ${this.areaToName(area)}: !!ERROR!! ${line.toUpperCase()}\x1b[0m`);
                            break;
                        }
                        case TraceWriter.WARNING: {
                            console.log(`\x1b[33m\x1b[1m ◉  TRACE ${this.areaToName(area)}: !WARNING! ${line.toUpperCase()}\x1b[0m`);
                            break;
                        }
                        case TraceWriter.INFO: {
                            console.log(`\x1b[${this.areaToColor(area)}m\x1b[1m ◉  TRACE ${this.areaToName(area)}: ${line.toUpperCase()}\x1b[0m`);
                            break;
                        }
    ...
    warn(line, area) { this.trace(line, TraceWriter.WARNING, area); }
    error(line, area) { this.trace(line, TraceWriter.ERROR, area); }
    info(line, area) { this.trace(line, TraceWriter.INFO, area); }
    verbose(line, area) { this.trace(line, TraceWriter.VERBOSE, area); }
}
...
area = area || 255;
var tracer = tracer || new TraceWriter(mode, category, area);
